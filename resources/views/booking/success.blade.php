<x-layout title="Booking Request Received">
    <x-slot:head>
    ...
    <section class="flex min-h-[70vh] items-center bg-navy-950 py-32">
        <div class="container-site max-w-2xl text-center">
            ...
            <h1 class="mt-8 font-display text-3xl font-semibold text-white sm:text-5xl" data-hero-reveal>Booking request received!</h1>
            <p class="mt-5 text-white/65" data-hero-reveal>
                Thanks {{ str($booking->name)->before(' ') }} — we've emailed a copy of your request to <strong class="text-white">{{ $booking->email }}</strong> and our team is checking availability now. You'll hear from us shortly.
            </p>
            <div class="mt-8 inline-block rounded-2xl border border-white/15 bg-white/5 px-8 py-5" data-hero-reveal>
                ...
                <p class="mt-1 font-display text-2xl font-bold text-accent-400">{{ $booking->reference }}</p>
            </div>
